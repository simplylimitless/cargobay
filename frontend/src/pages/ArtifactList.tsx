...
interface UpstreamSearchResult {
  namespace: string
  name: string
  description: string
  starCount: number
...
export function ArtifactList() {
  ...
  const isDocker = artifactType === 'docker'

  // Docker-type registries: search local cache + upstream registry
  const [upstreamResults, setUpstreamResults] = useState<UpstreamSearchResult[]>([])
  const [upstreamAvailable, setUpstreamAvailable] = useState(true)
  const [upstreamLoading, setUpstreamLoading] = useState(false)

  useEffect(() => {
    if (!isDocker || !registryId) return

    setUpstreamLoading(true)
    const controller = new AbortController()
    const timer = setTimeout(() => {
      const params = new URLSearchParams({ registry: registryId, type: 'docker', q: searchQuery })
      fetch(`/api/search/upstream?${params}`, { signal: controller.signal })
        .then((res) => res.json())
        .then((data) => {
          setUpstreamResults(data.results ?? [])
          setUpstreamAvailable(data.upstreamAvailable ?? false)
          setUpstreamLoading(false)
        })
        .catch((err) => {
          if (err.name !== 'AbortError') setUpstreamLoading(false)
        })
    }, 350)
  ...
  }, [isDocker, registryId, searchQuery])

  // npm/maven: static mock list, filtered client-side
  const staticArtifacts = !isDocker ? mockData[artifactType ?? ''] ?? [] : []
  const filteredStaticArtifacts = staticArtifacts.filter((artifact) =>
    artifact.name.toLowerCase().includes(searchQuery.toLowerCase()) ||
    artifact.description?.toLowerCase().includes(searchQuery.toLowerCase())
  )

  const loading = isDocker ? upstreamLoading : false
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="card">
        ...
        {isDocker && !upstreamAvailable && !upstreamLoading && (
          <div className="mb-6 px-4 py-3 bg-amber-500/10 border border-amber-500/30 rounded-lg text-amber-400 text-sm">
            Anonymous upstream search isn't available for <span className="font-mono">{registryId}</span> — showing locally cached results only.
          </div>
        )}
        ...
        {loading ? (
          ...
          <div className="space-y-3">
            {upstreamResults.length === 0 ? (
              ...
              upstreamResults.map((artifact) => {
                const fullName = `${artifact.namespace}/${artifact.name}`
                return (
                  <div
                    key={fullName}
                    onClick={() => handleArtifactClick(fullName)}
                      ...
                      <div>
                        <div className="flex items-center gap-3 mb-1">
                          ...
                          <span className="px-2 py-0.5 bg-gray-800 rounded text-xs text-gray-500">
                            {artifact.source === 'upstream' ? 'UPSTREAM' : 'CACHED'}
                          </span>
                          {artifact.official && (
