...
type TrackingAdapter struct {
	StorageAdapter
	onCacheHit func(registryID, namespace, artifactName, version string, bytesServed int64)
}

// NewTrackingAdapter wraps inner so onCacheHit is called with the identity and
// byte size of every artifact successfully served from local storage.
func NewTrackingAdapter(inner StorageAdapter, onCacheHit func(registryID, namespace, artifactName, version string, bytesServed int64)) *TrackingAdapter {
...
// identity and size to onCacheHit on success.
